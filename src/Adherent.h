...
public:
    ...
    void emprunte(int code);                                     // ok
    void rend(int code);
    friend ostream &operator<<(ostream &out, const Adherent &a); // ok
    void affiche();
    static vector<Adherent> initVecteurAdherent(vector<Bibliotheque> *liste_biblio); // ok
    static void enregistrerVecteurAdherent(vector<Adherent> listeadh); // ok
};
